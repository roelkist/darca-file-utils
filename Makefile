...
ifdef CI
    ...
    export VENV_PATH := /tmp/darca-file-venv
    export POETRY_HOME := /tmp/poetry-file-cache
    export POETRY_CONFIG_DIR := /tmp/poetry-file-config
    export POETRY_CACHE_DIR := $(POETRY_HOME)
    export POETRY_VIRTUALENVS_PATH := $(VENV_PATH)
    export PYTHONPYCACHEPREFIX := /tmp/file-pycache
    ...
    export PRE_COMMIT_HOME := /tmp/precommit-file-cache
endif
